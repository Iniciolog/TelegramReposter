...
export class AIContentAnalyzer {
  ...
  private async performAIAnalysis(htmlContent: string, url: string) {
    try {
      const prompt = `
ПРОАНАЛИЗИРУЙ ВСЕ БЛОКИ, ВКЛАДКИ И ЭЛЕМЕНТЫ САЙТА НА ПРЕДМЕТ НАЛИЧИЯ ТЕКСТА И МЕДИА.

ТВОЯ ЗАДАЧА:
1. Изучи все скрытые блоки, выпадающие меню, вкладки, модальные окна
2. Извлеки весь текстовый контент из всех найденных элементов
3. Найди все изображения и медиа-файлы
4. Оцени общий объем материала для Telegram поста

КРИТЕРИИ АВТОМАТИЧЕСКОГО ОДОБРЕНИЯ:
- Текст: более 100 символов = автоматически 85-95 баллов
- Текст: 50-100 символов = автоматически 70-80 баллов  
- Изображения: 1+ картинок = +10 баллов к оценке
- Смешанный контент (текст + медиа) = 90-100 баллов

ВСЕ МАТЕРИАЛЫ, СООТВЕТСТВУЮЩИЕ КРИТЕРИЯМ ОБЪЕМА, ПОЛУЧАЮТ АВТОМАТИЧЕСКОЕ ОДОБРЕНИЕ!
...
{
  "isValuable": true (если есть достаточно контента),
  "valueScore": число от 70 до 100 (оценка по объему материала),
  "title": "заголовок из контента",
  "description": "краткое описание найденного материала",
  "cleanContent": "весь извлеченный текст",
  "tags": ["теги по тематике"],
  "valuableImages": ["URL изображений"],
  "contentType": "article|news|blog|mixed"
}
`;
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Ты эксперт по анализу веб-контента для Telegram. Анализируй ВСЕ элементы страницы: блоки, вкладки, скрытые секции. Материалы с достаточным объемом текста (50+ символов) автоматически получают высокие оценки (70-100 баллов). Будь внимательным к деталям и извлекай максимум контента."
          },
          {
            role: "user",
            content: prompt
