...
export class AIContentAnalyzer {
  ...
  private async performAIAnalysis(htmlContent: string, url: string) {
    try {
      const prompt = `
ПРОСТО ОЧИСТИ И ОЦЕНИ ТЕКСТ!

ЕСЛИ В ТЕКСТЕ ЕСТЬ ХОТЯ БЫ 50+ СЛОВ - СТАВЬ 75-85 БАЛЛОВ!
ЕСЛИ МЕНЬШЕ 50 СЛОВ - СТАВЬ 45-65 БАЛЛОВ!

НЕ АНАЛИЗИРУЙ ГЛУБОКО! ТВОЯ ЗАДАЧА:
1. Извлечь основной текст
2. Поставить высокую оценку если есть текст
3. Очистить от HTML
...
ОТВЕТ В JSON (БЕЗ ОБЪЯСНЕНИЙ!):
{
  "isValuable": true (почти всегда!),
  "valueScore": 75-85 (для текста 50+ слов) или 50-65 (для короткого),
  "title": "заголовок",
  "description": "краткое описание",
  "cleanContent": "весь найденный текст",
  "tags": ["общие теги"],
  "valuableImages": [],
  "contentType": "article"
}
`;
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "ЗАДАЧА: быстро оценить текст. Если есть текст больше 30 слов = 75+ баллов. Меньше 30 слов = 50+ баллов. Только пустые страницы = низкие баллы. БУДЬ ЩЕДРЫМ!"
          },
          {
            role: "user",
            content: prompt
